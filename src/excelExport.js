...
export function exportToExcel(data, groupBy, selectedOnly) {
  ...
  const summaryHeader = [
    ...
    [],
    [getGroupLabel(groupBy), "Số lượng", "Thể tích (m³)", "Khối lượng (kg)"],
  ];
  ...
  for (const key of sortedKeys) {
    ...
    summaryRows.push([key, g.count, roundNum(g.volume, 6), roundNum(g.weight, 2)]);
  }

  summaryRows.push([]);
  summaryRows.push(["TỔNG CỘNG", data.length, roundNum(totalVolume, 6), roundNum(totalWeight, 2)]);
  ...
  const detailHeader = [
    ...
    [`Ngày xuất: ${dateStr}`],
    [],
    ["STT", "Tên", "Assembly", "Group", "Loại (IFC Type)", "Vật liệu", "Thể tích (m³)", "Khối lượng (kg)"],
  ];
...
function createGroupSheet(data, groupBy, label) {
  ...
  const rows = [
    ...
    [],
    [label, "Số lượng", "Thể tích (m³)", "Khối lượng (kg)"],
  ];
  ...
  for (const key of Object.keys(groups).sort()) {
    ...
    totalVol += g.volume;
    totalWt += g.weight;
    rows.push([key, g.count, roundNum(g.volume, 6), roundNum(g.weight, 2)]);
  }

  rows.push([]);
  rows.push(["TỔNG CỘNG", data.length, roundNum(totalVol, 6), roundNum(totalWt, 2)]);

  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws["!cols"] = [{ wch: 35 }, { wch: 12 }, { wch: 18 }, { wch: 18 }];
